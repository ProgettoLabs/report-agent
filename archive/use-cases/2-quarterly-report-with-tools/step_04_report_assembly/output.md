...
- **Structure:** One assembled report combining all prior step outputs in the
  order specified by the spec.  No new content — only assembly and formatting.

You MUST use `write_step_output` with `content_type="file"` and
`filename="final_report.md"` to persist the report to disk.

When including data from CSV outputs of previous steps, convert the CSV rows
into markdown tables.  The data values must not change.
...
[growth_highlights and flagged_items from step 2 reproduced here as prose or bullet lists]

---

## Appendix — Full Data

[All structured data tables from step 1 reproduced here as markdown tables]
