...
The Ollama settings are conservative:
...
When an OpenAI key is available, the script uses \texttt{gpt-4o-mini} with the same deterministic temperature setting.
...
\textbf{No key -- Ollama fallback:} if \texttt{OPENAI\_API\_KEY} is blank in \texttt{.env} and no \texttt{--api-key} flag is given, the script automatically uses the local Ollama model. Install Ollama from \url{https://ollama.com/download} and run:
