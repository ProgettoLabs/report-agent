...
\texttt{MODEL\_TYPE} chooses the model provider. Keep it as \texttt{"ollama"} for the local Ollama model, or set it to \texttt{"openai"} and fill in \texttt{OPENAI\_API\_KEY}.
...
This helper keeps the rest of the pipeline independent of the model provider. Both branches return a LangChain chat model with the same \texttt{invoke(...)} interface.
...
When \texttt{MODEL\_TYPE = "openai"}, the script uses \texttt{gpt-5.4-mini} with the same deterministic temperature setting.
...
By default, the script uses Ollama:
...
To use OpenAI instead, change the model configuration in \texttt{week-1/agent.py}:
