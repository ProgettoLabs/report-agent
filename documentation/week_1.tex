...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{mutedtext}Agentic AI Implementation Guide: Prof. Brian Subirana, April 30th, 2026}
...
\begin{titlepage}
  ...
  {\Huge\bfseries\color{darktext} Multi-step Agentic AI Parser: Quarterly Report Agent and beyond}\\[0.5em]
  {\Large\color{mutedtext} Prof. Brian Subirana \\ MIT Designing AI Products And Services Course}
  ...
  {\large\color{mutedtext}
    ...
    and the use-case folder format it executes. This is what was covered in the first webinar.
  }
...
In this document we describe the implementation discussed in the first webinar. In subsequent documents we'll cover webinar two and three. We'll call them "week 1", "week 2" and "week 3" respectively.

\subsection{The value proposition: simplify repetitive knowledge work}

The first webinar exercise was based in a scenario where we imagined you are an analyst at a corporation. Every quarter, your manager asks you to produce a report on the company's performance. The raw numbers change -- revenue, headcount, customer counts, expenses -- but the \textit{shape} of the job does not. You have done it before, and you will do it again next quarter, and the quarter after that.
...
The logic above and also be applied in other contexts as described in the webinar. In particular, we introduced examples taken from human resources (\textit{hiring evaluation}), marketing (\textit{sales call analysis}), strategy (\textit{market-entry analysis agent} and legal (\textit{contract review})

\begin{notebox}
\textbf{This is the pattern automated by the implementation we'll install.} Not the thinking that goes into designing the process -- that stays with you -- but the mechanical execution of each step, once the recipe is written down.
\end{notebox}
...
As discussed in the associated webinar, there is research showing improvements when a well detailed process is designed by a human, and no major improvement if you ask an llm to write the instructions for each step.
...
If we can write those three things clearly for one step, we can hand that step to anyone -- a junior colleague, an intern, or, as it turns out, a language model -- and expect a usable result. We do not need to explain \textit{how} to do the step. We just need to specify the goal, the rules, and the shape of the output.

\subsection{From a list of steps to a runnable pipeline}

Now we take that list of steps and give each one its own folder on your computer. Inside each folder, we put two short text files:
...
At the top level, alongside the step folders, we put one file with the raw numbers for this quarter (\texttt{input\_data.md}) and one file with a short description of the overall task (\texttt{agent\_task\_description.md}).

That is the whole setup. We have turned a vague ``write the quarterly report'' ask into a stack of small, individually-reviewable instructions. Anyone can read the folder and understand what the pipeline does, because it is all plain English.

\begin{notebox}
\textbf{The framework is the execution layer.} Once the step folders exist, a small Python script walks through them in order, hands each one's instructions to a language model, and threads the outputs together. In Week 1, the script reads those files directly from disk. The design work -- deciding what the steps are and what each one should produce -- is the part that requires your judgment. The script just runs the recipe.
\end{notebox}
...
The Week 1 pipeline is deliberately simple and powerful: steps run one after another, the agent reads local files, and a human kicks the whole thing off. That is enough for the quarterly report, but it is not the whole story.

In Week 2, you add MCP so the same assets can be exposed as shared resources instead of being read only as local files. This means you can have the best report writer in the world share its expertise with many agents. We will see in week2 how upgraded versions can add tools, remote data sources, hosted execution, or scheduled runs. The important point is that the step design does not need to change just because the execution environment becomes more capable.

In Week 3, we will look at a final extension using OpenClaw which allowsautonomous agents to schedule and run scripts at your convenience.
...
In Week 1's coude we introduce a small agent runner that executes a repeatable report-writing workflow (or any of the others introduced). The runner receives a single command-line argument, such as \texttt{"quarterly report"}, lists the available use cases, and asks the language model to resolve that input to the closest exact folder name.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item The agent calls the configured LangChain chat model for each step.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item Build the LangChain messages, call the configured chat model, and record the response.
...
report-agent/
|-- .env                      (OpenAI key, committed with blank value)
|-- week-1/
|   +-- agent.py
|
+-- use-cases/
...
In this Annex we describe the code at a high level and in the next one how to install it on your computer.
...
\begin{itemize}[leftmargin=1.5em]
  \item \texttt{argparse} to parse the command-line use-case argument and optional \texttt{--api-key} flag.
  ...
  \item \texttt{os} to read environment variables after loading the \texttt{.env} file.
  \item \texttt{Path} to work with local files and directories.
  \item \texttt{load\_dotenv} and \texttt{set\_key} from \texttt{python-dotenv} to read and update \texttt{.env}.
  \item \texttt{BaseChatModel} to type the shared chat model interface.
  \item \texttt{ChatOllama} to call a local model through LangChain.
  \item \texttt{ChatOpenAI} to call OpenAI through LangChain.
  \item \texttt{SystemMessage} and \texttt{HumanMessage} to build chat-style prompts.
\end{itemize}

\texttt{ENV\_PATH} points at the shared \texttt{.env} file in the repository root. The script reads the OpenAI API key from there; if a key is passed on the command line it is written back to that file for future runs.

\subsection{Loading the API key}
...
\texttt{load\_api\_key} handles the three cases in order. If a key was passed on the command line it becomes the source of truth and is written back to \texttt{.env} so future runs pick it up automatically. If no flag was given, the key already in \texttt{.env} is used. If neither source provides a key, \texttt{None} is returned and the agent falls back to Ollama.
...
This helper keeps the rest of the pipeline independent of the model provider. Both branches return a LangChain chat model with the same \texttt{invoke(...)} interface. The model is chosen automatically: OpenAI when a key is available, Ollama otherwise.
...
The default Ollama settings are conservative:
...
When an OpenAI key is available, the script uses \texttt{gpt-5.4-mini} with the same deterministic temperature setting.

\clearpage

\section{Setup Instructions}

\subsection{1. Install dependencies and run code if you use an OpenAI token}

The first thing you need to do is open the \texttt{agent.py} file and add your OpenAI token. This is a token you can get when you purchase a subscription to ChatGPT.

Once you CD into the directory where you unzip the code provided, really there are only the following four terminal commands to create a virtual environment, install the packages used by Week 1, and run the code:

\begin{lstlisting}[style=shellstyle]
python3 -m venv venv-agentic-webinars
source venv-agentic-webinars/bin/activate
pip install -r requirements.txt
python week-1/agent.py "quarterly report"
\end{lstlisting}

If you are using the repository's full \texttt{requirements.txt}, these dependencies are already included.

\subsection{2. OpenAI key configuration}
The repository includes a \texttt{.env} file at the root with a blank \texttt{OPENAI\_API\_KEY} entry. You have two ways to provide a key:

\textbf{Option A -- edit \texttt{.env} directly:}

\begin{lstlisting}[style=plainstyle]
OPENAI_API_KEY=your-api-key
\end{lstlisting}

\textbf{Option B -- pass it on the command line:}

\begin{lstlisting}[style=shellstyle]
python week-1/agent.py "quarterly report" --api-key your-api-key
\end{lstlisting}

The \texttt{--api-key} flag always takes precedence and writes the key back to \texttt{.env} so subsequent runs pick it up without needing the flag again.

\subsection{[Optional] Configure the model if you want to run a local model}

By default, the script uses Ollama if you have not provided the OpenAI API keys:

Install Ollama from \url{https://ollama.com/download}, then make sure the Ollama server is running:

\begin{notebox}
    If you already have ollama installed, it is still advisable to run \texttt{brew install ollama} since it will update it to the latest version.
\end{notebox}
...
\subsubsection{2.1 Run the agent}
