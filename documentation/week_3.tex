...
tabsize=2,
upquote=true,
morekeywords={python,pip,source,ollama,curl,bash}
}
...
\begin{warnbox}
OpenClaw will create the agent's instruction files inside this directory. If you ever decide to delete the agent, take a backup of your use-case folders first -- removing the agent will remove this entire directory.
\end{warnbox}
...
The agent has been created. Now we need to initialise it. Run the following command in your terminal:
...
While a simple ``Hi'' would work with more capable models, the message above is the standard OpenClaw greeting and is a safe choice across all model sizes. From here, follow the same persona setup process as you did with the main agent.
...
You can interact with the report agent through either the GUI or the TUI.
...
\textbf{Using the TUI:} if you closed the terminal from the previous step, reopen the session with:
