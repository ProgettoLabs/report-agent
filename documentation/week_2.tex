...
Week 1 needs LangChain model integrations for Ollama and OpenAI, plus \texttt{python-dotenv} for the shared \texttt{.env} file. Week 2 adds FastMCP:

\begin{lstlisting}[style=shellstyle]
pip install -r requirements.txt
\end{lstlisting}

If you use the repository's \texttt{requirements.txt}, these dependencies are already included.
...
\texttt{AGENT\_TYPE} is the default read channel. It is used when no \texttt{--agent-type} flag is passed on the command line. Keep it as \texttt{"local"} to behave like Week 1. Set it to \texttt{"mcp"} to read through the MCP server. The CLI flag always takes precedence over this global when provided.

\texttt{ENV\_PATH} points at the shared \texttt{.env} file in the repository root, the same file used by Week 1.
...
\subsection{API key loading and model initialization}

Week 2 uses the same key-loading and model-initialization helpers as Week 1:
...
The model choice is independent from the asset channel. For example, an OpenAI key can be used with either \texttt{AGENT\_TYPE = "local"} or \texttt{AGENT\_TYPE = "mcp"}.
...
\begin{lstlisting}[style=pythonstyle]
async def run_pipeline_with_client(
    use_case_input: str, api_key: str | None, agent_type: str = AGENT_TYPE
) -> str:
    global _mcp_client
...
\texttt{agent\_type} defaults to the global \texttt{AGENT\_TYPE} but can be overridden by the CLI. In local mode, no MCP client is created. In MCP mode, the client is opened once for the full pipeline run and then cleared in the \texttt{finally} block.
...
\subsection{Providing an OpenAI API key}

The repository includes a \texttt{.env} file at the root with a blank \texttt{OPENAI\_API\_KEY} entry. Model selection works the same way as Week 1:

\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Edit \texttt{.env} directly} with your key, or
  \item \textbf{pass \texttt{--api-key}} on the command line (it is saved back to \texttt{.env} automatically).
  \item \textbf{No key} -- the agent falls back to the local Ollama model.
\end{itemize}

\subsection{Local mode}

With \texttt{AGENT\_TYPE = "local"} (the default in the code), the agent reads files directly, like Week 1:

\begin{lstlisting}[style=shellstyle]
python week-2/agent.py "quarterly report"
python week-2/agent.py "quarterly report" --api-key your-key
\end{lstlisting}

You also have the option to pass the command line argument \texttt{--agent-type local} to set the agent as local:

\begin{lstlisting}[style=shellstyle]
python week-2/agent.py "quarterly report" --agent-type local
python week-2/agent.py "quarterly report" --agent-type local --api-key your-key
\end{lstlisting}

\subsection{MCP mode}

Pass \texttt{--agent-type mcp} on the command line, or set \texttt{AGENT\_TYPE = "mcp"} in \texttt{week-2/agent.py} to make it the default:

\begin{lstlisting}[style=shellstyle]
python week-2/agent.py "quarterly report" --agent-type mcp
\end{lstlisting}

The pipeline behavior is identical from the user's point of view. Internally, the agent reads task assets through the MCP resources instead of direct file reads.

\begin{notebox}
\textbf{CLI flag takes precedence.} If \texttt{--agent-type} is passed, it overrides the \texttt{AGENT\_TYPE} global regardless of what is set in the file. Omitting the flag falls back to the global.
\end{notebox}
...
Model provider & OpenAI or Ollama (auto) & OpenAI or Ollama (auto) \\
Key source & \texttt{.env} or \texttt{--api-key} flag & \texttt{.env} or \texttt{--api-key} flag \\
...
Channel switch & -- & \texttt{AGENT\_TYPE} global or \texttt{--agent-type} CLI flag \\
New dependency & \texttt{python-dotenv} & \texttt{fastmcp} \\
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Forgetting to install \texttt{python-dotenv}.} Both weeks need it to read and write the shared \texttt{.env} file.
  \item \textbf{Setting \texttt{AGENT\_TYPE} to an unsupported value.} It must be exactly \texttt{"local"} or \texttt{"mcp"}. The same applies to the \texttt{--agent-type} CLI flag; passing any other value is rejected by \texttt{argparse}.
  \item \textbf{Leaving \texttt{OPENAI\_API\_KEY} blank and expecting OpenAI.} If the key is absent in both \texttt{.env} and the \texttt{--api-key} flag, the agent silently falls back to Ollama.
