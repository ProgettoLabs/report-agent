...
Week 1 needs LangChain model integrations for Ollama and OpenAI. Week 2 adds FastMCP:

\begin{lstlisting}[style=shellstyle]
pip install langchain-ollama langchain-openai fastmcp
\end{lstlisting}
...
Week 2 uses the same model initialization helper as Week 1:
...
The model choice is independent from the asset channel. For example, \texttt{MODEL\_TYPE = "openai"} can run with either \texttt{AGENT\_TYPE = "local"} or \texttt{AGENT\_TYPE = "mcp"}.
...
Model provider & Ollama or OpenAI & Ollama or OpenAI \\
...
Mode switch & \texttt{MODEL\_TYPE} & \texttt{MODEL\_TYPE} and \texttt{AGENT\_TYPE} \\
New dependency & \texttt{langchain-openai} for OpenAI mode & \texttt{fastmcp} \\
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Confusing \texttt{MODEL\_TYPE} and \texttt{AGENT\_TYPE}.} \texttt{MODEL\_TYPE} chooses \texttt{"ollama"} or \texttt{"openai"}; \texttt{AGENT\_TYPE} chooses \texttt{"local"} or \texttt{"mcp"}.
